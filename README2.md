...
Quite often, the same filtered list of items is used in multiple in your class.  In this case, it's useful to create a convenience function to encapsulate the filtering logic:
...
Filtering can be nested too, however, the return type can become quite verbose. In this case, it's recommended to use `auto` as the return type (if your compiler will allow):

```cpp
auto valid_things() {
    return lib::filter(things, [](const auto &thing) { return thing.is_valid(); });
}
auto valid_things_with_name_bob() {
    return lib::filter(valid_things(), [](const auto &thing) { return thing.name() == "Bob"; });
}
```

Keeping verbose compiler types and possibly complex lambda expressions out of your business logic improves readability.